...
func (w *wsTransport) Dial(ctx context.Context, endpoint string, opts ...warp.DialOption) (warp.Connection, error) {
	cfg := &wsDialCfg{serializer: wio.NewMsgPackSerializer()}
	for _, o := range opts {
...
func (w *wsTransport) Listen(ctx context.Context, endpoint string, acceptor warp.Acceptor, opts ...warp.ListenOption) (warp.Listener, error) {
	cfg := &wsListenCfg{
		path:       "/ws",
		serializer: wio.NewMsgPackSerializer(),
		upgrader: websocket.Upgrader{
	...
	mux.HandleFunc(path, func(wr http.ResponseWriter, r *http.Request) {
		...
		case "json":
			ser = wio.NewJSONSerializer()
		case "msgpack":
			ser = wio.NewMsgPackSerializer()
		}
		ws := &wsConnection{
			c:         c,
			ser:       ser,
...
func (c *wsConnection) Serializer() warp.Serializer     { return c.ser }
func (c *wsConnection) SessionID() int64                { return c.sessionID }
func (c *wsConnection) SetSerializer(s warp.Serializer) { c.ser = s }
...
func (c *wsConnection) Send(ctx context.Context, msg *warp.WMsg) error {
	...
	data, err := c.ser.Marshal(msg) // encode WMsg with active serializer
	if err != nil {
	...
	go func() {
		...
		writeErr := c.c.WriteMessage(c.ser.MessageType(), data)
		if !ok {
...
func (c *wsConnection) startReadLoop() {
	...
	go func() {
		...
		for {
			...
			if mt == c.ser.MessageType() {
				deser = c.ser
			} else {
				if s, ok := wio.TryCreateSerializerFor(mt); ok {
					deser = s
			...
			if err := deser.Unmarshal(data, &msg); err != nil {
				if c.handler != nil {
					c.handler.OnError(c, fmt.Errorf("unmarshal: %w", err))
				}
				continue
			}
			if c.handler != nil {
